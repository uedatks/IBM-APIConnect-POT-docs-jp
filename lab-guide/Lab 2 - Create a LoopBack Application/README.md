...
このラボでは、いくつかの API リソース操作を介して製品アイテムにアクセスするための新規アプリケーションを作成します。
...
**ThinkIBM** は、歴史的価値がある希少な IBM 機器を取り扱う販売会社です。 **ThinkIBM** は 複数の API を利用して、製品アイテム・データベースへのアクセスを簡素化したいと考えていました。また、このアプリケーションに購入者がレビューを残す機能を持たせることも必要です。このラボの受講者の目的はアプリケーション開発者として、この製品アイテムへアクセスするためのアプリケーションを作成することです。
...
## 2.2 - 製品アイテム・アプリケーションの作成

API Connect Developer ツールキットに含まれる LoopBack フレームワークを使って製品アイテム・アプリケーションを作成します。Web フレームワークである Express フレームワーク上で稼働し、Swagger 2.0 仕様に対応している LoopBack を使用することで、スケーラブルな API を迅速に作成できます。LoopBack は、高い機能拡張性を持った、オープンソースの Node.js のフレームワークです。
...
API はデータ・ソースを使ってバックエンドのデータ・レポジトリーとやりとりします。今回は、MySQL を使用し、製品アイテムのデータを保管します。
...
1. 現在のディレクトリーが `~/ThinkIBM/inventory` であることを確認し、以下のコマンドを入力します:
	...
	Firefox ウェブ・ブラウザが起動し、API Designer 画面が自動的にロードされます。
...
3. 「新規ループバック・モデル」ダイアログに、モデル名として `item` を入力し、`新規作成` ボタンをクリックします。
...
MySQL データベースの `item` 表には、7 の列があり、これらもマッピングが必要です。`item` モデルのプロパティーを作成する手順は以下のとおりです: 
...
9. 終了したら、API `探索` に戻り、`操作の呼び出し` ボタンを再度クリックします。
...
12. 同じ手順で、`GET /items/count` 操作もテストします。12 件の製品アイテムがカウントとして返されるはずです。
...
13. 画面下の `Stop` ボタンをクリックして、製品アイテム・アプリケーションと MicroGateway を停止します。

	![](../img/lab2/stop-application_jp.png)

## 2.8 - MongoDB データ・ソースの作成

ここまでで、MySQL データベースに保存された製品アイテムに関する API を提供する LoopBack アプリケーションを作成しました。

次のセクションでは、製品アイテムに対するレビューのデータ・モデルを作成します。レビュー・データは MongoDB に保存されます。

まずは、MongoDB データ・ソースを作成します。

1. Firefox のタブもしくはウィンドウの `x` ボタンをクリックし、ブラウザーを閉じます。

2. `apic edit` を実行した `Terminal` に戻ります。
...
5. ターミナルが、新規データ・ソースの構成ウィザードを呼び出します。構成ウィザードでは、いくつかの質問が表示されます。テキスト入力を必要とする質問のほか、事前に定義された選択肢から回答を選ぶような質問もあります。
	...
	Connector-spedific configuration:
	...
	>
	> 実質的には、以下を実行し、MongoDB コネクターをインストールするのと同じことです:
	>
...
`review` データ・モデルは、購入者が残した製品アイテムに対するレビューを保管するために使用します。レビューは MongoDB に保管されます。
...
2. `review` モデルのプロパティーを入力します:
	...
	> ![][important]
	> 
	> review モデルは REST API として公開**しません**。後ほど製品アイテムとレビューを関連付けますが、reviewテーブル自体を公開対象とはしません。
...
8. 最後に製品アイテムの `rating` プロパティーを追加します:
...
5. `apic edit` を実行した `Terminal` に戻ります。
...
# Lab 2 - 検証
```
TODO：検証はバージョンがあがっていて、古いバージョンで作成したものとは異なる
```
