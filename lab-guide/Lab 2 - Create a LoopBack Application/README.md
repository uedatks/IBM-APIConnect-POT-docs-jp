...
1. 現在のディレクトリーが `~/ThinkIBM/inventory` であることを確認し、以下のコマンドを入力します:
	...
	Firefox ウェブブラウザーが起動し、API Designer 画面が自動的にロードされます。
...
2. タスクバーから `Terminal` を選択し、コマンドラインを開きます。
...
2. `review` モデルのプロパティーを入力します:
	...
	> ![][important]
	> 
	> review モデルは REST API として公開**しません**。後ほど製品アイテムとレビューを関連付けますが、この作業の一環で使用する REST API を作成します。
